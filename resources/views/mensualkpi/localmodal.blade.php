          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">{{$kpi->nombre}} por mes: {{$formato}}, local {{$local}}</h3>
              </div>
              ...
              <div class="card-header border-0">
                ...
                <div class="card-tools">
                <form action="{{ route('mensualkpis.modal', [ 'kpiid'=>$kpi->id , 'formato'=>$formato , 'local' => $local  ]) }}" method="GET" class="navbar-form navbar-left" id="myform" name="myform">
                    
                    <a class="btn btn-secondary btn-sm" role="button" id="exportpdf" title="Reporte PDF">
                      <i class="far fa-file-pdf"></i>
                       
                    </a>
                    <a  class="btn btn-secondary btn-sm" href="{{ route('exportExcelMensual', [ 'kpiid'=>$kpi->id , 'formato'=>$formato , 'local' => $local, 'archivoExcel' => $kpi->nombre. ' por dia '. $formato .' '. $local .' ' . date('d-m-Y')  ]) }}" role="button" title="Exportar Excel">
                    ...
                    <input id="endDate" name="endDate" type="hidden" >
                     ...
                     <a type="submit" class="btn btn-secondary btn-sm" role="button" id="daterange-btn" title="Rango Meses"> 
                    <i class="far fa-calendar-alt"></i>
                      Rango Meses
                    </a> 
                    <button type="submit" class="btn btn-default btn-sm" id="hinddenBtn" style=" visibility: hidden"> 
                      
                    </button> 
                    
                  
                    <button type="button" class="btn bg-info btn-sm"  id="changeColumn" title="Gráfico Barras">
                    <i class="fas fa-chart-bar"></i>
                    </button>
                    <button type="button" class="btn bg-info btn-sm"  id="changeSpLine" title="Gráfico líneas">
              ...
              <div class="card-body">
                ...
                <table id="example1xx" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <td>{{ date("m-Y", strtotime($locale->mes)) }}</td>
                        
                        <td><span class=" bg-danger">{{ $locale->valor }} {{ $kpi->formato }}</span>
...
Highcharts.exportCharts = function(charts, options) {
    ...
    Highcharts.post(options.url, {
      filename: options.filename || '{{$kpi->nombre}} por mes',
      type: options.type,
      width: options.width,
...
<script type="text/javascript">
  ...
  $(function () {
    ...
    $('#daterange-btn').daterangepicker(
      ...
      function (start, end) {
        if ( start != '') {
        $('#startDate').val(start.format('YYYY-MM-01') );
        $('#endDate').val(end.format('YYYY-MM-01') );
